...
int32_t             gGamma1000;                 // In-game tweakable gamma setting where 1000 = no adjustment (1.0)
...
static void loadPrefsFileIniEntry(const IniUtils::IniEntry& entry) noexcept {
    if (entry.key == "gamma") {
        gGamma1000 = std::clamp(entry.value.tryGetAsInt(gGamma1000), GAMMA_MIN, GAMMA_MAX);
    }
...
void setToDefaults() noexcept {
    ...
    // Gamma and turn speed is normal by default, auto-run off and no stat display
    gGamma1000 = GAMMA_DEFAULT;
